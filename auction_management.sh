...
echo "AuctionHouse_V14 Management Running..."
echo "Signer:" $2
...
while true; do
	...
	if [ `expr $LAST_HEIGHT \< $CURRENT_HEIGHT` == 1 ]; then
		AUCTIONS=$(flow-o json events get A.01837e15023c9249.AuctionHouse_V14.ItemWon --last $(expr $CURRENT_HEIGHT - $LAST_HEIGHT))  # emulator
