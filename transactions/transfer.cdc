...
transaction(signer: Address, withdrawID: UInt64) {
    ...
    let transferToken: @DAAM_V4.NFT
	
    prepare(acct: AuthAccount) {

        // Borrow a reference from the stored collection
        let collectionRef = acct.borrow<&DAAM_V4.Collection{NonFungibleToken.Provider}>(from: DAAM_V4.collectionStoragePath)
            ?? panic("Could not borrow a reference to the owner's collection")

        // Call the withdraw function on the sender's Collection
        // to move the NFT out of the collection
        self.transferToken <- collectionRef.withdraw(withdrawID: withdrawID) as! @DAAM_V4.NFT
    }
    ...
    execute {        
        ...
        let receiverCap = recipient.getCapability<&DAAM_V4.Collection{NonFungibleToken.Receiver}>(DAAM_V4.collectionPublicPath)
        let receiverRef = receiverCap.borrow()! //as &DAAM_V4.Collection //{NonFungibleToken.CollectionPublic}
