...
transaction(exAgent: Address) {
    ...
    pre { DAAM_V9.isAgent(exAgent) != nil : exAgent.toString().concat(" is not an Agent.") }
    
    execute {
        self.admin.removeAgent(agent: self.exAgent)
        log("Removed Agent")

        if DAAM_V9.isMinter(self.exAgent) != nil {
    ...
    post { DAAM_V9.isAgent(self.exAgent) == nil : self.exAgent.toString().concat(" is still an Agent.") }
