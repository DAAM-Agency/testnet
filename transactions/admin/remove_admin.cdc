...
transaction(exAdmin: Address)
{
    ...
    prepare(admin: AuthAccount) {
        self.admin = admin.borrow<&DAAM_V16.Admin>(from: DAAM_V16.adminStoragePath) ?? panic(exAdmin.toString().concat(" is not an Admin."))
	    self.exAdmin = exAdmin
    }

    // Verify exAdmin is an Admin
    pre { DAAM_V16.isAdmin(exAdmin) != nil : exAdmin.toString().concat(" is not an Admin.") }
