...
transaction(creator: Address, status: Bool) {
    ...
    prepare(agent: AuthAccount) {
        ...
        self.status  = status
        self.admin = agent.borrow<&{DAAM_V21.Agent}>(from: DAAM_V21.V21.adminStoragePath)!
    }

    pre { DAAM_V21.V21.isCreator(creator) != nil : creator.toString().concat(" is not a Creator.") }
