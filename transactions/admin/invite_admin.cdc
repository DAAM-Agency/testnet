...
transaction(newAdmin: Address)
{
    ...
    pre {
        DAAM_V16.isAdmin(newAdmin) == nil   : newAdmin.toString().concat(" is already an Admin.")
        DAAM_V16.isAgent(newAdmin) == nil   : newAdmin.toString().concat(" is already an Agent.")
        DAAM_V16.isCreator(newAdmin) == nil : newAdmin.toString().concat(" is already an Creator.")
    }
