...
transaction(newAdmin: Address)
{
    ...
    pre {
        DAAM_V18.isAdmin(newAdmin) == nil   : newAdmin.toString().concat(" is already an Admin.")
        DAAM_V18.isAgent(newAdmin) == nil   : newAdmin.toString().concat(" is already an Agent.")
        DAAM_V18.isCreator(newAdmin) == nil : newAdmin.toString().concat(" is already an Creator.")
    }
