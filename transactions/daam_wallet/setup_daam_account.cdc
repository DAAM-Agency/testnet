...
// Create A DAAM_V19 Wallet to store DAAM_V19 NFTs
...
transaction(public: Bool)
{
    ...
    prepare(acct: AuthAccount) {
        if acct.borrow<&DAAM_V19.Collection>(from: DAAM_V19.collectionStoragePath) != nil {
            panic("You already have a DAAM_V19 Collection.")
        }
    ...
    execute {
        let collection <- DAAM_V19.createEmptyCollection()    // Create a new empty collection
        self.acct.save<@NonFungibleToken.Collection>(<-collection, to: DAAM_V19.collectionStoragePath) // save the new account
        
        if self.public {
            self.acct.link<&{DAAM_V19.CollectionPublic, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, MetadataViews.Resolver}>(DAAM_V19.collectionPublicPath, target: DAAM_V19.collectionStoragePath)
            log("DAAM_V19 Account Created. You have a DAAM_V19 Collection (Public) to store NFTs'")
        } else {
            log("DAAM_V19 Account Created. You have a DAAM_V19 Collection (Non-Public) to store NFTs'")
        }
