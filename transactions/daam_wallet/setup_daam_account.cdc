...
transaction(public: Bool)
{
    ...
    prepare(acct: AuthAccount) {
        if acct.borrow<&DAAM.Collection>(from: DAAM_V22.collectionStoragePath) != nil {
    ...
    execute {
        ...
        if self.public {
            self.acct.link<&{DAAM.CollectionPublic, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, MetadataViews.Resolver}>(DAAM.collectionPublicPath, target: DAAM_V22.collectionStoragePath)
            log("DAAM Account Created. You have a DAAM_V22 Collection (Public) to store NFTs'")
        } else {
            log("DAAM Account Created. You have a DAAM_V22 Collection (Non-Public) to store NFTs'")
        }
