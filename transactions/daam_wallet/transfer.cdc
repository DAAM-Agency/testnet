...
transaction(signer: Address, withdrawID: UInt64) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let collectionRef = acct.borrow<&DAAM_V8.Collection{NonFungibleToken.Provider}>(from: DAAM_V8.collectionStoragePath)
    ...
    execute {        
        ...
        let receiverCap = recipient.getCapability<&DAAM_V8.Collection{NonFungibleToken.Receiver}>(DAAM_V8.collectionPublicPath)
        let receiverRef = receiverCap.borrow()! //as &DAAM_V8.Collection //{NonFungibleToken.CollectionPublic}
