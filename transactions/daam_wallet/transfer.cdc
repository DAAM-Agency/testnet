...
transaction(signer: Address, withdrawID: UInt64) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let collectionRef = acct.borrow<&DAAM_V9.Collection{NonFungibleToken.Provider}>(from: DAAM_V9.collectionStoragePath)
    ...
    execute {        
        ...
        let receiverCap = recipient.getCapability<&DAAM_V9.Collection{NonFungibleToken.Receiver}>(DAAM_V9.collectionPublicPath)
        let receiverRef = receiverCap.borrow()! //as &DAAM_V9.Collection //{NonFungibleToken.CollectionPublic}
