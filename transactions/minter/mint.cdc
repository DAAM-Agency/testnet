...
transaction(creator: Address, mid: UInt64, receiver: Address) {
    let minterRef : &DAAM_V7.Minter
    let creator   : Address
    let mid       : UInt64
    let metadataRef : &{DAAM_V7.MetadataGeneratorMint}
    let receiverRef : &{NonFungibleToken.CollectionPublic}

    prepare(minter: AuthAccount) {
        self.minterRef = minter.borrow<&DAAM_V7.Minter>(from: DAAM_V7.minterStoragePath)!
        self.creator   = creator
        self.mid       = mid

        self.receiverRef  = getAccount(receiver)
            .getCapability(DAAM_V7.collectionPublicPath)
            .borrow<&{NonFungibleToken.CollectionPublic}>()!


        self.metadataRef = getAccount(self.creator)
            .getCapability(DAAM_V7.metadataPublicPath)
            .borrow<&{DAAM_V7.MetadataGeneratorMint}>()!
    }
