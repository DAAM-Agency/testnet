...
transaction(submit: Bool) {
    ...
    execute {
        let admin <- DAAM_V18.answerAdminInvite(newAdmin: self.signer, submit: self.submit)
        if admin != nil {
            let old_admin <- self.signer.load<@AnyResource>(from: DAAM_V18.adminStoragePath)
            self.signer.save<@DAAM_V18.Admin>(<- admin!, to: DAAM_V18.adminStoragePath)
            let adminRef = self.signer.borrow<&DAAM_V18.Admin>(from: DAAM_V18.adminStoragePath)!
            destroy old_admin
            ...
            log("You are now a DAAM_V18.Admin: ".concat(self.signer.address.toString()) )
        } else {
            destroy admin
