...
transaction(submit: Bool) {
    ...
    execute {
        let admin <- DAAM_V17.answerAdminInvite(newAdmin: self.signer, submit: self.submit)
        if admin != nil {
            let old_admin <- self.signer.load<@AnyResource>(from: DAAM_V17.adminStoragePath)
            self.signer.save<@DAAM_V17.Admin>(<- admin!, to: DAAM_V17.adminStoragePath)
            let adminRef = self.signer.borrow<&DAAM_V17.Admin>(from: DAAM_V17.adminStoragePath)!
            destroy old_admin
            ...
            log("You are now a DAAM_V17.Admin: ".concat(self.signer.address.toString()) )
        } else {
            destroy admin
