...
transaction(submit: Bool) {
    ...
    execute {
        let agent  <- DAAM_V22.V22.answerAgentInvite(newAgent: self.signer, submit: submit)

        if agent != nil && submit {
<<<<<<< HEAD
            let old_admin <- self.signer.load<@AnyResource>(from: DAAM_V22.V22.adminStoragePath)
            self.signer.save<@DAAM_V22.Admin{DAAM_V22.Agent}>(<- agent!, to: DAAM_V22.V22.adminStoragePath)!
            let agentRef = self.signer.borrow<&{DAAM_V22.Agent}>(from: DAAM_V22.V22.adminStoragePath)!
=======
...
>>>>>>> 586a0096 (updated FUSD Address)
            ...
            log("You are now a DAAM_V22.V22.Agent: ".concat(self.signer.address.toString()) )
            
            // Minter
<<<<<<< HEAD
            if DAAM_V22.V22.isMinter(self.signer.address) == false { // Received Minter Invitation
                let old_minter <- self.signer.load<@AnyResource>(from: DAAM_V22.V22.minterStoragePath)
                let minter  <- DAAM_V22.V22.answerMinterInvite(newMinter: self.signer, submit: submit)
                self.signer.save<@DAAM_V22.Minter>(<- minter!, to: DAAM_V22.V22.minterStoragePath)
                log("You are now a DAAM_V22.V22.Minter: ".concat(self.signer.address.toString()) )
=======
