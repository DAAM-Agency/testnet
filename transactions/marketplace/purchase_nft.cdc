...
transaction(seller: Address, tokenID: UInt64, amount: UFix64) {
    // reference to the buyer's NFT collection where they will store the bought NFT
    let buyerCollectionRef: &AnyResource{NonFungibleToken.Receiver}
    ...
    execute {
        ...
        saleRef.purchase(tokenID: tokenID, recipient: self.buyerCollectionRef, buyTokens: <-self.temporaryVault)
