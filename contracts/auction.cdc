// auction.cdc
// by Ami Rajpal, 2021 // DAAM_V1 Agency
...
pub contract AuctionHouse {
    ...
    access(contract) var metadataGen : {UInt64 : Capability<&DAAM_V1.MetadataGenerator>} // {MID  :Capability<&DAAM_V1.MetadataGenerator>}
...
/************************************************************************/
    pub resource AuctionWallet {
        ...
        pub fun createOriginalAuction(metadataGenerator: Capability<&DAAM_V1.MetadataGenerator>, mid: UInt64, start: UFix64, length: UFix64,
        ...
        pub fun createAuction(nft: @DAAM_V1.NFT, start: UFix64, length: UFix64, isExtended: Bool,
...
/************************************************************************/
    pub resource Auction {
        ...
        pub var auctionNFT  : @DAAM_V1.NFT?
        priv var auctionVault: @FungibleToken.Vault
        // nft data
    
        init(nft: @DAAM_V1.NFT, start: UFix64, length: UFix64, isExtended: Bool, extendedTime: UFix64,
        ...
        access(contract) fun verifyReservePrice() {
            ...
            let collectionRef = getAccount(target!).getCapability<&{DAAM_V1.CollectionPublic}>(DAAM_V1.collectionPublicPath).borrow()!
        ...
        priv fun royality() {
            ...
            let agencyPercentage  = royality[DAAM_V1.agency]!
            let creatorPercentage = royality[metadataRef.creator]!

            let agencyRoyality  = DAAM_V1.newNFTs.contains(self.tokenID) ? 0.2 : agencyPercentage
            let creatorRoyality = DAAM_V1.newNFTs.contains(self.tokenID) ? 0.8 : creatorPercentage
            // If 1st sale set remove from 'new list'
            if DAAM_V1.newNFTs.contains(self.tokenID) { 
            ...
            let agencyPay  = getAccount(DAAM_V1.agency).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver).borrow()!
        ...
        pub fun getMetadataRef(): &DAAM_V1.Metadata {
            let ref = &self.auctionNFT?.metadata! as &DAAM_V1.Metadata
            return ref 
        }
...
// AuctionHouse Functions & Constructor
    access(contract) fun notNew(tokenID: UInt64) {
        let minter = self.account.borrow<&DAAM_V1.Minter>(from: DAAM_V1.minterStoragePath)!
        minter.notNew(tokenID: tokenID)
    }

    access(contract) fun mintNFT(metadata: @DAAM_V1.MetadataHolder): @DAAM_V1.NFT {
        let minter = self.account.borrow<&DAAM_V1.Minter>(from: DAAM_V1.minterStoragePath)!
    ...
    init() {
        self.metadataGen = {}
        self.auctionStoragePath = /storage/DAAM_V1_Auction
        self.auctionPublicPath  = /public/DAAM_V1_Auction
    }
}
