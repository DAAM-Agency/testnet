...
pub struct AuctionHolder {
        ...
        pub let buyNow        : UFix64   // buy now price (original price + AuctionHouse_V17.fee)
        ...
        init(
            ...
            )
            {
                ...
                self.buyNow        = buyNow   // buy now price (original price + AuctionHouse_V17.fee)
...
/************************************************************************/
    pub resource AuctionWallet: AuctionWalletPublic {
            ...
            startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): UInt64
        {
            ...
            AuctionHouse_V17.currentAuctions.insert(key: self.owner?.address!, self.currentAuctions.keys) // Update Current Auctions
            return aid
        }
            ...
            startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): @Auction
        {
            ...
            if metadataGenerator != nil {
                ...
                AuctionHouse_V17.metadataGen.insert(key: id, metadataGenerator!) // add access to Creators' Metadata
                let metadataRef = metadataGenerator!.borrow()! as &DAAM_V23.MetadataGenerator{DAAM_V23.MetadataGeneratorMint} // Get MetadataHolder
                let minterAccess <- AuctionHouse_V17.minterAccess(mid: id)
            ...
            for act in self.currentAuctions.keys {                
                ...
                if current_status == false { // Check to see if auction has ended. A false value.
                    ...
                    if self.currentAuctions.keys.length == 0 {
                        AuctionHouse_V17.currentAuctions.remove(key:self.owner!.address) // If auctioneer has no more auctions remove from list
                    } else {
                        AuctionHouse_V17.currentAuctions.insert(key:self.owner!.address, self.currentAuctions.keys) // otherwise update list with reset values
                    }
        ...
        priv fun validToken(vault: &FungibleToken.Vault): Bool {
            ...
            return AuctionHouse_V17.crypto.containsKey(identifier)
        }
    ...
    pub resource Auction: AuctionPublic {
        ...
        init(metadata: @DAAM_V23.Metadata?, nft: @DAAM_V23.NFT?, start: UFix64, length: UFix64, isExtended: Bool, extendedTime: UFix64, vault: @FungibleToken.Vault,
          incrementByPrice: Bool, incrementAmount: UFix64, startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?) {
            ...
            AuctionHouse_V17.auctionCounter = AuctionHouse_V17.auctionCounter + 1 // increment Auction Counter
            ...
            self.auctionID = AuctionHouse_V17.auctionCounter // Auction uinque ID number
            ...
            self.fee = AuctionHouse_V17.getFee(mid: self.mid)
        ...
        access(contract) fun verifyReservePrice() {
            ...
            if pass { // leader met the reserve price              
                if self.auctionMetadata != nil { // If Metadata turn into nFt
                    ...
                    let old <-  self.auctionNFT <- AuctionHouse_V17.mintNFT(metadata: <-metadata!)
                    destroy old
                }
                ...
                AuctionHouse_V17.updateAllHistory(mid: self.mid, id: id, metadata: metadata, history: saleHistoryEntry)
        ...
        priv fun payFirstSale() {
            ...
            let price       = self.auctionVault.balance / (1.0 + AuctionHouse_V17.getFee(mid: self.mid))
            let fee         = self.auctionVault.balance - price   // Get fee amount
            let creatorRoyalties = self.convertTo100Percent() // get Royalty data
            let daamRoyalty = AuctionHouse_V17.getAgencyFirstSale(mid: self.mid)
        ...
        priv fun royalty()
        {
            ...
            if DAAM_V23.isNFTNew(id: tokenID) {
                AuctionHouse_V17.notNew(tokenID: tokenID) 
                self.payFirstSale()
            } else {   // 2nd Sale
        ...
        priv fun seriesMinter() {
            ...
            let metadataRef = AuctionHouse_V17.metadataGen[self.mid]!.borrow()!   // get Metadata Generator Reference
            let minterAccess <- AuctionHouse_V17.minterAccess(mid: self.mid)
            let metadata <-! metadataRef.generateMetadata(minter: <- minterAccess)
            let old <- self.auctionNFT <- AuctionHouse_V17.mintNFT(metadata: <-metadata) // Mint NFT and deposit into auction
...
/************************************************************************/
// AuctionHouse_V17 Functions & Constructor
    ...
    init() {
        ...
        self.auctionStoragePath = /storage/DAAM_V23.Auction
        self.auctionPublicPath  = /public/DAAM_V23.Auction
