// auction.cdc
// by Ami Rajpal, 2021 // DAAM_V19 Agency
...
pub contract AuctionHouse_V12 {
    ...
    access(contract) var metadataGen     : {UInt64 : Capability<&DAAM_V19.MetadataGenerator{DAAM_V19.MetadataGeneratorMint}> }
...
pub struct SaleHistory {
    pub let metadata    : DAAM_V19.MetadataHolder
    pub var saleHistory : [SaleHistoryEntry]

    init(metadata: DAAM_V19.MetadataHolder) {
...
pub struct AuctionHolder {
        ...
        pub let creatorInfo   : DAAM_V19.CreatorInfo
        ...
        pub let buyNow        : UFix64   // buy now price (original price + AuctionHouse_V12.fee)
        ...
        init(
            _ status:Bool?, _ auctionID:UInt64, _ creator: DAAM_V19.CreatorInfo, _ mid: UInt64, _ start: UFix64, _ length: UFix64,
            ...
            )
            {
                ...
                self.buyNow        = buyNow   // buy now price (original price + AuctionHouse_V12.fee)
...
/************************************************************************/
    pub resource AuctionWallet: AuctionWalletPublic {
        ...
        // nft: DAAM_V19.NFT or nil when MetadataGenerator argument is entered
        // id: DAAM_V19 Metadata ID or Token ID depenedent whether nft or MetadataGenerator is entered
        ...
        pub fun createAuction(metadataGenerator: Capability<&DAAM_V19.MetadataGenerator{DAAM_V19.MetadataGeneratorMint}>?, nft: @DAAM_V19.NFT?, id: UInt64, start: UFix64,
            ...
            startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): UInt64
        {
            ...
            if metadataGenerator != nil {
                assert(DAAM_V19.getCopyright(mid: id) != DAAM_V19.CopyrightStatus.FRAUD, message: "This submission has been flaged for Copyright Issues.")
                assert(DAAM_V19.getCopyright(mid: id) != DAAM_V19.CopyrightStatus.CLAIM, message: "This submission has been flaged for a Copyright Claim.")

                AuctionHouse_V12.metadataGen.insert(key: id, metadataGenerator!) // add access to Creators' Metadata
                let metadataRef = metadataGenerator!.borrow()! as &DAAM_V19.MetadataGenerator{DAAM_V19.MetadataGeneratorMint} // Get MetadataHolder
                let minterAccess <- AuctionHouse_V12.minterAccess()
            ...
            AuctionHouse_V12.currentAuctions.insert(key:self.owner?.address!, self.currentAuctions.keys) // Update Current Auctions
            return aid
        }        
            ...
            for act in self.currentAuctions.keys {                
                ...
                if current_status == false { // Check to see if auction has ended. A false value.
                    ...
                    if self.currentAuctions.keys.length == 0 {
                        AuctionHouse_V12.currentAuctions.remove(key:self.owner!.address) // If auctioneer has no more auctions remove from list
                    } else {
                        AuctionHouse_V12.currentAuctions.insert(key:self.owner!.address, self.currentAuctions.keys) // otherwise update list with reset values
                    }
...
/************************************************************************/
    pub resource interface AuctionPublic {
        ...
        pub fun itemInfo(): DAAM_V19.MetadataHolder?
...
/************************************************************************/
    pub resource Auction: AuctionPublic {
        ...
        access(contract) var auctionMetadata : @DAAM_V19.Metadata? // Store NFT for auction
        access(contract) var auctionNFT : @DAAM_V19.NFT? // Store NFT for auction
        ...
        // reprintSeries: to duplicate the current auction, with a reprint (Next Mint os Series)
        // *** new is defines as "never sold", age is not a consideration. ***
        init(metadata: @DAAM_V19.Metadata?, nft: @DAAM_V19.NFT?, start: UFix64, length: UFix64, isExtended: Bool, extendedTime: UFix64, vault: @FungibleToken.Vault,
          incrementByPrice: Bool, incrementAmount: UFix64, startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?) {
            ...
            let metadataHolder: DAAM_V19.MetadataHolder = (metadata != nil) ? metadata?.getHolder()! : nft?.metadata!
            ...
            AuctionHouse_V12.auctionCounter = AuctionHouse_V12.auctionCounter + 1 // increment Auction Counter
            ...
            self.auctionID = AuctionHouse_V12.auctionCounter // Auction uinque ID number
            ...
            let ref = (nft != nil) ? &nft?.metadata! as &DAAM_V19.MetadataHolder : &metadata?.getHolder()! as &DAAM_V19.MetadataHolder
            ...
            self.fee = AuctionHouse_V12.getFee(mid: self.mid)
        ...
        access(contract) fun verifyReservePrice() {
            ...
            if pass { // leader met the reserve price              
                if self.auctionMetadata != nil { // If Metadata turn into nFt
                    ...
                    let old <-  self.auctionNFT <- AuctionHouse_V12.mintNFT(metadata: <-metadata!)
                    destroy old
                }
                ...
                AuctionHouse_V12.updateAllHistory(mid: self.mid, id: id, metadata: metadata, history: saleHistoryEntry)
                ...
                if self.auctionMetadata != nil { // return Metadata to Creator
                    let metadata <- self.auctionMetadata <- nil
                    let ref = getAccount(receiver!).getCapability<&DAAM_V19.MetadataGenerator{DAAM_V19.MetadataGeneratorPublic}>(DAAM_V19.metadataPublicPath).borrow()!
        ...
        priv fun finalise(receiver: Address?, nft: @DAAM_V19.NFT, pass: Bool) {
            log("receiver: ".concat(receiver!.toString()) )   
            let collectionRef = getAccount(receiver!).getCapability<&{NonFungibleToken.CollectionPublic}>(DAAM_V19.collectionPublicPath).borrow()!
        ...
        priv fun returnFunds() {
            ...
            for bidder in self.auctionLog.keys {
                ...
                let bidderRef =  getAccount(bidder).getCapability<&{FungibleToken.Receiver}>
                    (AuctionHouse_V12.getCrypto(crypto: self.requiredCurrency))
                    .borrow()!
        ...
        pub fun itemInfo(): DAAM_V19.MetadataHolder? { // returns the metadata of the item NFT.
        ...
        priv fun payFirstSale() {
            ...
            if self.auctionNFT?.metadata!.creatorInfo.agent == nil {
                ...
                self.payRoyalty(price: fee, royalties: DAAM_V19.agency.getRoyalties() ) // Fee Payment
        ...
        priv fun royalty()
        {
            ...
            if DAAM_V19.isNFTNew(id: tokenID) {
                AuctionHouse_V12.notNew(tokenID: tokenID) 
                self.payFirstSale()
            } else {   // 2nd Sale
                ...
                self.payRoyalty(price: fee * inHouse, royalties: [DAAM_V19.company] ) // get Comapny share of fee
                self.payRoyalty(price: fee * agency, royalties: DAAM_V19.agency.getRoyalties() ) // Pay Agency the fee
        ...
        priv fun seriesMinter() {
            ...
            let metadataRef = AuctionHouse_V12.metadataGen[self.mid]!.borrow()!   // get Metadata Generator Reference
            let minterAccess <- AuctionHouse_V12.minterAccess()
            let metadata <-! metadataRef.generateMetadata(minter: <- minterAccess, mid: self.mid)
            let old <- self.auctionNFT <- AuctionHouse_V12.mintNFT(metadata: <-metadata) // Mint NFT and deposit into auction
...
/************************************************************************/
// AuctionHouse_V12 Functions & Constructor

    // Sets NFT to 'not new' 
    access(contract) fun notNew(tokenID: UInt64) {
        let minter = self.account.borrow<&DAAM_V19.Minter>(from: DAAM_V19.minterStoragePath)!
    ...
    access(contract) fun mintNFT(metadata: @DAAM_V19.Metadata): @DAAM_V19.NFT {
        let minterRef = self.account.borrow<&DAAM_V19.Minter>(from: DAAM_V19.minterStoragePath)! // get Minter Reference
        let nft <- minterRef.mintNFT(metadata: <-metadata)! // Mint NFT
        return <- nft                                    // Return NFT
    }

    // Requires Minter Key // Minter function to mint
    access(contract) fun minterAccess(): @DAAM_V19.MinterAccess {
        let minterRef = self.account.borrow<&DAAM_V19.Minter>(from: DAAM_V19.minterStoragePath)! // get Minter Reference
        let minter_access <- minterRef.createMinterAccess()
        return <- minter_access                                  // Return NFT
    }

    access(contract) fun updateAllHistory(mid: UInt64, id: UInt64, metadata: DAAM_V19.MetadataHolder, history: SaleHistoryEntry) {
        // update saleHistory
    ...
    pub fun addFee(mid: UInt64, fee: UFix64, permission: &DAAM_V19.Admin) {
        pre { DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied" }
        self.fee[mid] = fee
    }

    pub fun removeFee(mid: UInt64, fee: UFix64, permission: &DAAM_V19.Admin) {
        pre {
            DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied"
    ...
    pub fun addAgencyFirstSale(mid: UInt64, fee: UFix64, permission: &DAAM_V19.Admin) {
        pre { DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied" }
        self.agencyFirstSale[mid] = fee
    }

    pub fun removeAgencyFirstSale(mid: UInt64, fee: UFix64, permission: &DAAM_V19.Admin) {
        pre {
            DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied"
            self.fee[mid] != nil : "No set Fee for this MID."
        }
    ...
    pub fun addCrypto(crypto: &FungibleToken.Vault, path: PublicPath, permission: &DAAM_V19.Admin) {
        pre { DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied" }
    ...
    pub fun removeCrypto(crypto: String, permission: &DAAM_V19.Admin) {
        pre {
            DAAM_V19.isAdmin(permission.owner!.address) == true : "Permission Denied"
    ...
    init() {
        ...
        self.auctionStoragePath = /storage/DAAM_V19_Auction
        self.auctionPublicPath  = /public/DAAM_V19_Auction
