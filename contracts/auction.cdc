    ...
    pub resource AuctionWallet: AuctionWalletPublic {
            ...
            startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): @Auction
        {
            ...
            if metadataGenerator != nil {
                assert(DAAM.getCopyright(mid: id) != DAAM_V22.CopyrightStatus.FRAUD, message: "This submission has been flaged for Copyright Issues.")
                assert(DAAM.getCopyright(mid: id) != DAAM_V22.CopyrightStatus.CLAIM, message: "This submission has been flaged for a Copyright Claim.")
...
// AuctionHouse_V15 Functions & Constructor
    ...
    access(contract) fun notNew(tokenID: UInt64) {
        let minter = self.account.borrow<&DAAM.Minter>(from: DAAM_V22.minterStoragePath)!
    ...
    access(contract) fun mintNFT(metadata: @DAAM.Metadata): @DAAM.NFT {
        let minterRef = self.account.borrow<&DAAM.Minter>(from: DAAM_V22.minterStoragePath)! // get Minter Reference
