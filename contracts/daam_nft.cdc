...
pub contract DAAM_Mainnet: NonFungibleToken {
    ...
    pub let agency : MetadataViews.Royalties  // DAAM_Mainnet Agency Founder Royaly Addresses
    pub let company: MetadataViews.Royalty    // DAAM_Mainnet Company Address
/***********************************************************************/
...
pub resource Request {
    ...
    init(mid: UInt64) {
        pre { mid !=0 && mid <= DAAM_Mainnet.metadataCounterID : "Illegal Operation: validate" }

        self.mid = mid          // Get Metadata ID
        DAAM_Mainnet.metadata[self.mid] != false // Can set a Request as long as the Metadata has not been Disapproved as oppossed to Aprroved or Not Set.
...
pub resource RequestGenerator {
    ...
    pub fun createRequest(mid: UInt64, royalty: MetadataViews.Royalties ) {
        pre {
            !DAAM_Mainnet.request.containsKey(mid) : "Already made request for this MID."

            DAAM_Mainnet.isCreator(self.owner!.address) == true || DAAM_Mainnet.isAdmin(self.owner!.address) == true
            : "You do not have access"
        }
        let request <-! create Request(mid: mid)!
        let old <- DAAM_Mainnet.request.insert(key: mid, <-request) // advice DAAM_Mainnet of request
        destroy old
    ...
    pub fun acceptDefault(mid: UInt64, metadataGen: &MetadataGenerator{MetadataGeneratorPublic}, royalties: MetadataViews.Royalties) {
        pre {
            ...
            DAAM_Mainnet.creators.containsKey(self.grantee) : "Account: ".concat(self.grantee.toString()).concat("You are not a Creator")
            DAAM_Mainnet.isCreator(self.grantee) == true    : "Account: ".concat(self.grantee.toString()).concat("Your Creator account is Frozen.")
            //percentage >= 0.1 && percentage <= 0.3  : "Percentage must be inbetween 10% to 30%."
        }

        // Getting Agency royalties
        let agency   = DAAM_Mainnet.agency.getRoyalties()
        let rate     = 0.025
        ...
        for creator in creators {
            ...
            if !DAAM_Mainnet.creatorHistory.containsKey(creator.receiver.address) {
                DAAM_Mainnet.creatorHistory[creator.receiver.address] = [mid]
            } else if !DAAM_Mainnet.creatorHistory[creator.receiver.address]!.contains(mid) {
                DAAM_Mainnet.creatorHistory[creator.receiver.address]!.append(mid)
            }
        }
...
/************************************************************************/
    pub resource Metadata {  // Metadata struct for NFT, will be transfered to the NFT.
        ...
        {            
            pre {
                DAAM_Mainnet.validInteract(interact) : "This Interaction is not Authorized"
            ...
            if metadata == nil {
                DAAM_Mainnet.metadataCounterID = DAAM_Mainnet.metadataCounterID + 1
                self.mid         = DAAM_Mainnet.metadataCounterID // init MID with counter
                self.creatorInfo = creator!               // creator of NFT
...
pub resource MetadataGenerator: MetadataGeneratorPublic, MetadataGeneratorMint {
        ...
        init(_ grantee: Address) {
            ...
            DAAM_Mainnet.metadataCap.insert(key: self.grantee, getAccount(self.grantee).getCapability<&MetadataGenerator{MetadataGeneratorPublic}>(DAAM_Mainnet.metadataPublicPath))
        }
            ...
            misc: String, thumbnail: {String:{MetadataViews.File}}, file: {String:MetadataViews.Media}, interact: AnyStruct? ): UInt64
        {
            ...
            let metadata <- create Metadata(creator: DAAM_Mainnet.creators[self.grantee], name: name, max: max, categories: categories,
        ...
        access(contract) fun saveMID(mid: UInt64)  {
            DAAM_Mainnet.metadata.insert(key: mid, false)   // a metadata ID for Admin approval, currently unapproved (false)
            DAAM_Mainnet.copyright.insert(key: mid, CopyrightStatus.UNVERIFIED) // default copyright setting

            DAAM_Mainnet.metadata[mid] = true // TODO REMOVE AUTO-APPROVE AFTER DEVELOPMENT
        ...
        pub fun removeMetadata(mid: UInt64) {
            ...
            let old_request <- DAAM_Mainnet.request.remove(key: mid)  // Get Request
        ...
        priv fun clearMetadata(mid: UInt64): @Metadata {         
            pre { mid !=0 && mid <= DAAM_Mainnet.metadataCounterID : "Illegal Operation: validate" }
            DAAM_Mainnet.metadata.remove(key: mid) // Metadata removed from DAAM_Mainnet. Logging no longer neccessary
            DAAM_Mainnet.copyright.remove(key:mid) // remove metadata copyright            
        ...
        pub fun generateMetadata(minter: @MinterAccess) : @Metadata {
            pre {
                ...
                DAAM_Mainnet.metadata[minter.mid] != nil : "MetadataID: ".concat(minter.mid.toString()).concat(" This already has been published.")
                DAAM_Mainnet.metadata[minter.mid]!       : "MetadataID: ".concat(minter.mid.toString()).concat(" Submission has been Disapproved.")
            }
            let mid = minter.mid
            destroy minter
...
/************************************************************************/
    pub resource NFT: NonFungibleToken.INFT, INFT, MetadataViews.Resolver {
        ...
        init(metadata: @Metadata, request: &Request?) {
            ...
            DAAM_Mainnet.totalSupply = DAAM_Mainnet.totalSupply + 1 // Increment total supply
            self.id          = DAAM_Mainnet.totalSupply     // Set Token ID with total supply
        ...
        pub fun getCopyright(): CopyrightStatus { // Get current NFT Copyright status
            return DAAM_Mainnet.copyright[self.id]! // return copyright status
        }
...
pub struct NFTCollectionDisplay: CollectionDisplay {
    ...
    pub fun addMID(creator: &Creator, mid: UInt64, feature: Bool) {
        pre  {
            mid != 0 && mid <= DAAM_Mainnet.metadataCounterID : "Illegal Operation on MID"
            !self.mid.containsKey(mid)       : "Already is in this Collection."
            DAAM_Mainnet.isCreator(creator.grantee) == true   : "You are not a Creator or your account is Frozen."
            DAAM_Mainnet.creatorHistory[creator.grantee]!.contains(mid) : "This MID does not belong to this Creator."
        }
    ...
    pub fun removeMID(creator: &Creator, mid: UInt64) {
        pre {
            mid != 0 && mid <= DAAM_Mainnet.metadataCounterID : "Illegal Operation: removeMID (pre)" 
...
pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic,
   ...
   pub fun removeCollection(name: String) {
        ...
        switch view {
            /*
                return MetadataViews.NFTCollectionData (
                    storagePath: DAAM_Mainnet.collectionStoragePath,
    ...
    pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT {
        let token <- self.ownedNFTs.remove(key: withdrawID)! as! @DAAM_Mainnet.NFT // Get NFT
    ...
    pub fun deposit(token: @NonFungibleToken.NFT) {
        let token <- token as! @DAAM_Mainnet.NFT // Get NFT as DAAM_Mainnet.GFT
    ...
    pub fun depositByAgent(token: @NonFungibleToken.NFT, name: String, feature: Bool, permission: &Admin{Agent}) {
        pre { DAAM_Mainnet.getAgentCreators(agent: permission.grantee)!.contains(self.owner?.address!) : "Permission Denied." }
...
pub resource Admin: Agent
{
        ...
        pub fun newRequestGenerator(): @RequestGenerator {
            pre {
                DAAM_Mainnet.admins[self.owner!.address] == true  : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied. No proper Admin Status")
                self.grantee == self.owner!.address       : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
        ...
        pub fun inviteAdmin(_ admin: Address) {     // Admin invite a new Admin
            pre {
                DAAM_Mainnet.admins[self.owner!.address] == true  : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
                self.grantee == self.owner!.address : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
            ...
            DAAM_Mainnet.admins.insert(key: admin, false) // Admin account is setup but not active untill accepted.
        ...
        pub fun inviteAgent(_ agent: Address) {    // Admin ivites new Agent
            ...
            post {
                DAAM_Mainnet.agents[agent] == false : "Illegal Operaion: invite Agent"
                DAAM_Mainnet.admins[agent] == false : "Illegal Operaion: invite Agent"
            }

            DAAM_Mainnet.admins.insert(key: agent, false) // Admin account is setup but not active untill accepted.
            DAAM_Mainnet.agents.insert(key: agent, false )     // Agent account is setup but not active untill accepted.
        ...
        pub fun inviteCreator(_ creator: Address, agentCut: UFix64 ) {    // Admin or Agent invite a new creator, agentCut = nil no agent
            pre {
                DAAM_Mainnet.admins[self.owner!.address] == true  : "Permission Denied"
            ...
            DAAM_Mainnet.creators.insert(key: creator,  creatorInfo) // Creator account is setup but not active untill accepted.
        ...
        pub fun inviteMinter(_ minter: Address) {   // Admin invites a new Minter (Key)
            ...
            post { DAAM_Mainnet.minters[minter] == false : "Illegal Operaion: inviteCreator" }

            DAAM_Mainnet.minters.insert(key: minter, false) // Minter Key is setup but not active untill accepted.
        ...
        pub fun removeAdmin(admin: Address) { // Two Admin to Remove Admin
            ...
            DAAM_Mainnet.remove.insert(key: self.grantee, admin) // Append removal list
            if DAAM_Mainnet.remove.length >= vote {                      // If votes is 3 or greater
                var counter: {Address: Int} = {} // {To Remove : Total Votes}
                // Talley Votes
                for a in DAAM_Mainnet.remove.keys {
                    let remove = DAAM_Mainnet.remove[a]! // get To Remove
                    // increment counter
                ...
                for c in counter.keys {
                    if counter[c]! >= vote {        // Does To Remove have enough votes to be removed
                        DAAM_Mainnet.remove = {}           // Reset DAAM_Mainnet.Remove
                        DAAM_Mainnet.admins.remove(key: c) // Remove selected Admin
                        log("Removed Admin")
        ...
        pub fun removeAgent(agent: Address) { // Admin removes selected Agent by Address
            pre  {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                    : "You're no longer a have Access."
                DAAM_Mainnet.agents.containsKey(agent) : "This is not a Agent Address."
            }
            post {
                !DAAM_Mainnet.admins.containsKey(agent) : "Illegal operation: removeAgent"
                !DAAM_Mainnet.agents.containsKey(agent) : "Illegal operation: removeAgent"
            } // Unreachable

            DAAM_Mainnet.admins.remove(key: agent)    // Remove Agent from list
            DAAM_Mainnet.agents.remove(key: agent)    // Remove Agent from list
            log("Removed Agent")
        ...
        pub fun removeCreator(creator: Address) { // Admin removes selected Creator by Address
            pre {  
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                        : "You're no longer a have Access."
                DAAM_Mainnet.creators.containsKey(creator) : "This is not a Creator address."
                DAAM_Mainnet.isAgent(self.owner!.address) == nil || // Verify Agent or Admin
                DAAM_Mainnet.agentHistory[self.owner!.address]!.contains(creator) : "Access Denied!"
            }
            post { !DAAM_Mainnet.creators.containsKey(creator) : "Illegal operation: removeCreator" } // Unreachable

            DAAM_Mainnet.creators.remove(key: creator)    // Remove Creator from list
            DAAM_Mainnet.metadataCap.remove(key: creator) // Remove Metadata Capability from list
            log("Removed Creator")
        ...
        pub fun removeMinter(minter: Address) { // Admin removes selected Agent by Address
            pre {
                DAAM_Mainnet.isAdmin(self.owner!.address) == true  : "Permission Denied"
                self.grantee == self.owner!.address        : "Permission Denied"
                self.status                      : "You're no longer a have Access."
                DAAM_Mainnet.isMinter(minter) != nil     : "This is not a Minter Address."
            }
            post { !DAAM_Mainnet.minters.containsKey(minter) : "Illegal operation: removeAgent" } // Unreachable
            DAAM_Mainnet.minters.remove(key: minter)    // Remove Agent from list
            log("Removed Minter")
        ...
        pub fun changeAgentStatus(agent: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                     : "You're no longer a have Access."
                DAAM_Mainnet.agents.containsKey(agent)  : "Wrong Address. This is not an Agent."
                DAAM_Mainnet.agents[agent] != status    : "Agent already has this Status."
            }
            post { DAAM_Mainnet.agents[agent] == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            DAAM_Mainnet.agents[agent] = status // status changed
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                         : "You're no longer a have Access."
                DAAM_Mainnet.creators.containsKey(creator)  : "Wrong Address. This is not a Creator."
                DAAM_Mainnet.isCreator(creator) != status    : "Agent already has this Status."
                DAAM_Mainnet.isAgent(self.owner!.address) == nil || // Verify Agent or Admin
                DAAM_Mainnet.agentHistory[self.owner!.address]!.contains(creator) : "Access Denied!"
            }
            post { DAAM_Mainnet.isCreator(creator) == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            DAAM_Mainnet.creators[creator]!.setStatus(status) // status changed
        ...
        pub fun changeMinterStatus(minter: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                       : "You're no longer a have Access."
                DAAM_Mainnet.minters.containsKey(minter)  : "Wrong Address. This is not a Minter."
                DAAM_Mainnet.minters[minter] != status    : "Minter already has this Status."
            }
            post { DAAM_Mainnet.minters[minter] == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            DAAM_Mainnet.minters[minter] = status // status changed
        ...
        pub fun changeMetadataStatus(creator: Address, mid: UInt64, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address  : "Permission Denied"
                self.status                          : "You're no longer a have Access."
                DAAM_Mainnet.copyright.containsKey(mid)      : "This is an Invalid MID"
                DAAM_Mainnet.isAgent(self.owner!.address) == nil || // Verify Agent or Admin
                DAAM_Mainnet.agentHistory[self.owner!.address]!.contains(creator) : "Access Denied!"
                DAAM_Mainnet.creatorHistory[creator]!.contains(mid) : "This Creator does not have this MID"
            }            
                
            DAAM_Mainnet.metadata[mid] = status // change to a new Metadata status
        }      
        ...
        pub fun changeCopyright(creator: Address, mid: UInt64, copyright: CopyrightStatus) {
            ...
            post { DAAM_Mainnet.copyright[mid] == copyright  : "Illegal Operation: changeCopyright" } // Unreachable

            DAAM_Mainnet.copyright[mid] = copyright    // Change to new copyright
        ...
        pub fun createMetadata(creator: Address, name: String, max: UInt64?, categories: [Categories.Category], description: String,
            ...
            let metadata <- create Metadata(creator: DAAM_Mainnet.creators[creator], name: name, max: max, categories: categories,
                description: description, misc: misc, thumbnail: thumbnail, interact: interact, file: file, metadata: nil) // Create Metadata
            let mid = metadata.mid
        ...
        pub fun bargin(creator: Address, mid: UInt64, percentage: UFix64) {
            ...
            let request <- DAAM_Mainnet.request.remove(key: mid)!
            request.bargin(isCreator: false, mid: mid, royalty: royalty)
            let old <- DAAM_Mainnet.request[mid] <- request
            destroy old
        }
	}
/************************************************************************/
...
// to Create Metadata which inturn can be made in NFTs after Minting
    pub resource Creator {
        ...
        pub fun newMetadataGenerator(): @MetadataGenerator {
            pre{
                self.grantee == self.owner!.address : "Permission Denied"
                DAAM_Mainnet.creators.containsKey(self.grantee) : "You're not a Creator."
                DAAM_Mainnet.isCreator(self.grantee) == true    : "Your Creator account is Frozen."
            }
...
// Note: new is defined by newly Minted. Age is not a consideration.
    pub resource Minter
    {
        ...
        init(_ minter: Address) {
            self.grantee = minter
            DAAM_Mainnet.minters.insert(key: minter, true) // Insert new Minter in minter list.
        }

        pub fun mintNFT(metadata: @Metadata): @DAAM_Mainnet.NFT {
            pre{
                DAAM_Mainnet.isCreator(metadata.creatorInfo.creator) == true :
                    "Account: ".concat(metadata.creatorInfo.creator.toString()).concat(" This is not our Creator.")
                DAAM_Mainnet.isMinter(self.grantee) == true      : "Account: ".concat(self.grantee.toString()).concat(" Your Creator account is Frozen.")
                DAAM_Mainnet.request.containsKey(metadata.mid)   : "Invalid Request for MID: ".concat(metadata.mid.toString())
            }
            var isLast = false
            ...
            if isLast {
                let request <- DAAM_Mainnet.request.remove(key: mid)! // Get Request using MID
        ...
        pub fun notNew(tokenID: UInt64) {
            pre  {
                self.grantee == self.owner!.address : "Permission Denied"
                DAAM_Mainnet.newNFTs.contains(tokenID)      : "Token ID: ".concat(tokenID.toString()).concat(" is Not New.")
            }
            post { !DAAM_Mainnet.newNFTs.contains(tokenID) : "Illegal Operation: notNew" } // Unreachable

            var counter: UInt64 = 0 as UInt64              // start the conter
            for nft in DAAM_Mainnet.newNFTs {              // cycle through 'new' list
                if nft == tokenID {                // if Token ID is found
                    DAAM_Mainnet.newNFTs.remove(at: counter) // remove from 'new' list
                    break
                } else {
        ...
        priv fun newNFT(id: UInt64) {
            pre  { !DAAM_Mainnet.newNFTs.contains(id) : "Token ID: ".concat(id.toString()).concat(" is already set to New.") }
            post { DAAM_Mainnet.newNFTs.contains(id)  : "Illegal Operation: newNFT" }
                DAAM_Mainnet.newNFTs.append(id)       // Append 'new' list
        }        
    }
...
pub resource MinterAccess 
{
    ...
    pub fun validate(creator: Address): Bool {
        pre {
            DAAM_Mainnet.isMinter(self.minter) == true : "You access has been denied."
            DAAM_Mainnet.isCreator(creator)    == true : creator.toString().concat(" is not a Creator or account is Frozen.")     
        }

        if DAAM_Mainnet.isAgent(self.minter) == nil   { return true  } // Is a Marketplace, Access Approvd, minterStatus is always true
        if DAAM_Mainnet.isAgent(self.minter) == false { return false } // Access is Frozen or Invitation has not been acccepted.
        if DAAM_Mainnet.isAgent(self.minter) == true  {  // is an Agent, verify creator & mid relationship to agent
                // Verify MID belongs to Creator && Verify Agent is Creators'
                let valid_mid = DAAM_Mainnet.getCreatorMIDs(creator: creator)!.contains(self.mid)
        		let is_creators_agent = DAAM_Mainnet.agentHistory[self.minter]!.contains(creator)
...
/************************************************************************/
    ...
    pub fun answerAdminInvite(newAdmin: AuthAccount, submit: Bool): @Admin? {
        ...
        if !submit { 
            DAAM_Mainnet.admins.remove(key: newAdminAddress) // Release Admin
            return nil
        }  // Refused invitation. Return and end function
        
        // Invitation accepted at this point
        DAAM_Mainnet.admins[newAdminAddress] = submit // Insert new Admin in admins list.
        log("Admin: ".concat(newAdminAddress.toString()).concat(" added to DAAM_Mainnet") )
    ...
    pub fun answerAgentInvite(newAgent: AuthAccount, submit: Bool): @Admin{Agent}?
    {
        ...
        if !submit {                                  // Refused invitation. 
            DAAM_Mainnet.admins.remove(key: newAgentAddress) // Remove potential from Agent list
            DAAM_Mainnet.agents.remove(key: newAgentAddress) // Remove potential from Agent list
            return nil                                // Return and end function
        }
        // Invitation accepted at this point
        DAAM_Mainnet.admins[newAgentAddress] = submit        // Add Admin & set Status (True)
        DAAM_Mainnet.agents[newAgentAddress] = submit        // Add Agent & set Status (True)
        DAAM_Mainnet.agentHistory[newAgentAddress] = []     // Setup Agent History


        log("Agent: ".concat(newAgentAddress.toString()).concat(" added to DAAM_Mainnet") )
    ...
    pub fun answerCreatorInvite(newCreator: AuthAccount, submit: Bool): @Creator? {
        ...
        if !submit {                                       // Refused invitation.
            DAAM_Mainnet.creators.remove(key: newCreatorAddress)  // Remove potential from Agent list
            return nil                                     // Return and end function
        }
        // Invitation accepted at this point
        DAAM_Mainnet.creators[newCreatorAddress]!.setStatus(submit) // Add Creator & set Status (True)

        let agent = DAAM_Mainnet.creators[newCreatorAddress]!.agent
        log("Agent: ".concat(agent.toString()) )
        
        // Update agent History with Creator Address
        if DAAM_Mainnet.agentHistory[agent] == nil {
            DAAM_Mainnet.agentHistory[agent] = [newCreatorAddress]
        } else if !DAAM_Mainnet.agentHistory[agent]!.contains(newCreatorAddress) {
            DAAM_Mainnet.agentHistory[agent]!.append(newCreatorAddress)
        }

        log("Creator: ".concat(newCreatorAddress.toString()).concat(" added to DAAM_Mainnet") )
    ...
    pub fun answerMinterInvite(newMinter: AuthAccount, submit: Bool): @Minter? {
        ...
        if !submit {                                      // Refused invitation. 
            DAAM_Mainnet.minters.remove(key: newMinterAddress) // Remove potential from Agent list
            return nil                                    // Return and end function
        }
        // Invitation accepted at this point
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        ...
        return <- create DAAM_Mainnet.Collection() // Return Collection Resource
    }
   ...
   pub fun isAdmin(_ admin: Address): Bool? { // Returns Admin Status
    ...
    pub fun isCreator(_ creator: Address): Bool? { // Returns Creator status
        return DAAM_Mainnet.creators[creator]?.status // nil = not a creator, false = invited to be a creator, true = is a creator
    }
...
/************************************************************************/
// Init DAAM_Mainnet Contract variables
    ...
    init(founders: {Address:UFix64}, company: Address, defaultAdmins: [Address])
    {
        ...
        // Paths
        self.collectionPublicPath  = /public/DAAM_Mainnet_Collection
        self.collectionStoragePath = /storage/DAAM_Mainnet_Collection
        self.metadataPublicPath    = /public/DAAM_Mainnet_SubmitNFT
        self.metadataStoragePath   = /storage/DAAM_Mainnet_SubmitNFT
        self.adminPrivatePath      = /private/DAAM_Mainnet_Admin
        self.adminStoragePath      = /storage/DAAM_Mainnet_Admin
        self.minterPrivatePath     = /private/DAAM_Mainnet_Minter
        self.minterStoragePath     = /storage/DAAM_Mainnet_Minter
        self.creatorPrivatePath    = /private/DAAM_Mainnet_Creator
        self.creatorStoragePath    = /storage/DAAM_Mainnet_Creator
        self.requestPrivatePath    = /private/DAAM_Mainnet_Request
        self.requestStoragePath    = /storage/DAAM_Mainnet_Request
