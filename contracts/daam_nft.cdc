...
pub contract DAAM_V3: NonFungibleToken
{
    ...
    access(contract) var creatorCap: {Address: Capability<&DAAM_V3.MetadataGenerator> } // {Address : Capability of Metadata}
...
pub resource Request {
    ...
    init(metadata: &Metadata) {
        ...
        DAAM_V3.metadata[self.mid] != false // Can set a Request as long as the Metadata has not been Disapproved as oppossed to Aprroved or Not Set.
...
pub resource RequestGenerator {
    ...
    pub fun getRequest(metadata: &MetadataHolder): @Request {
        pre {
            ...
            DAAM_V3.request.containsKey(metadata.getMID() )     : "No Request made."
            DAAM_V3.getRequestValidity(mid: metadata.getMID() ) : "This request has not been approved."
        }
        let request <- DAAM_V3.request.remove(key: metadata.metadata.mid)! // Remove Request
...
/************************************************************************/
    pub struct Metadata {  // Metadata struct for NFT, will be transfered to the NFT.
        ...
        init(creator: Address, series: UInt64, data: String, thumbnail: String, file: String, counter: UInt64) {
            ...
            self.mid       = DAAM_V3.metadataCounterID // init MID with counter
...
pub resource MetadataGenerator: MetadataGeneratorPublic { 
        ...
        pub fun addMetadata(series: UInt64, data: String, thumbnail: String, file: String) {
            ...
            DAAM_V3.metadataCounterID = DAAM_V3.metadataCounterID + 1 as UInt64  // Must be first, increment Metadata Countert
            ...
            DAAM_V3.metadata.insert(key: metadata.mid, false)   // a metadata ID for Admin approval, currently unapproved (false)
        ...
        pub fun removeMetadata(mid: UInt64) {
            ...
            let old_request <- DAAM_V3.request.remove(key: mid)  // Get Request
        ...
        priv fun deleteMetadata(mid: UInt64) {
            ...
            DAAM_V3.copyright.remove(key:mid) // remove metadata copyright            
...
/************************************************************************/
    pub resource NFT: NonFungibleToken.INFT, INFT {
        ...
        init(metadata: @MetadataHolder, request: &Request) {
            ...
            DAAM_V3.totalSupply = DAAM_V3.totalSupply + 1 as UInt64 // Increment total supply
            self.id = DAAM_V3.totalSupply                        // Set Token ID with total supply
        ...
        pub fun getCopyright(): CopyrightStatus { // Get current NFT Copyright status
            return DAAM_V3.copyright[self.id]! // return copyright status
        }
    }
/************************************************************************/
...
pub resource interface CollectionPublic {
    ...
    pub fun borrowDAAM(id: UInt64): &DAAM_V3.NFT            // get NFT as DAAM_V3.NFT
}     
/************************************************************************/
// Standand Flow Collection Wallet
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @DAAM_V3.NFT // Get NFT as DAAM_V3.GFT
        ...
        // borrowDAAM gets a reference to an DAAM_V3.NFT in the collection.
        pub fun borrowDAAM(id: UInt64): &DAAM_V3.NFT {
            ...
            return ref as! &DAAM_V3.NFT                                    // return NFT Reference
        }        
    ...
    pub resource interface Agent 
    {
        ...
        pub fun inviteCreator(_ creator: Address) {  // Admin invites a new creator
            pre {
                ...
                DAAM_V3.admins[creator]   == nil : "A Creator can not use the same address as an Admin."
                DAAM_V3.agents[creator]   == nil : "A Creator can not use the same address as an Agent."
                DAAM_V3.creators[creator] == nil : "They're already a DAAM Creator!!!"
                Profile.check(creator) : "You can't be a DAAM Creator without a Profile! Go make one Fool!!"
            }
            post { DAAM_V3.creators[creator] == false : "Illegal Operaion: inviteCreator" }
        }
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            pre {
                self.status                         : "You're no longer a have Access."
                DAAM_V3.creators.containsKey(creator)  : "Wrong Address. This is not a Creator."
                DAAM_V3.creators[creator] != status    : "Creator already has this Status."
            }
            post { DAAM_V3.creators[creator] == status : "Illegal Operation: changeCreatorStatus" } // Unreachable
        }
        
        pub fun removeCreator(creator: Address) {      // Admin or Agent can remove Creator
            pre  {
                self.status                        : "You're no longer a have Access."
                DAAM_V3.creators.containsKey(creator) : "This is not a Creator address."
            }
            post { !DAAM_V3.creators.containsKey(creator) : "Illegal operation: removeCreator" } // Unreachabel
        }        
        ...
        pub fun changeCopyright(mid: UInt64, copyright: CopyrightStatus) {
            pre  {
                self.status                 : "You're no longer a have Access."
                DAAM_V3.copyright.containsKey(mid)  : "This is an Invalid MID"
            }
            post { DAAM_V3.copyright[mid] == copyright : "Illegal Operation: changeCopyright" } // Unreachable
        }
...
pub resource Admin: Agent
{
        ...
        init(_ admin: AuthAccount) {
            ...
            DAAM_V3.admins.insert(key: admin.address, true) // Insert new Admin in admins list.
        }
        ...
        pub fun inviteAdmin(newAdmin: Address) { // Admin invite a new Admin
            DAAM_V3.adminPending = newAdmin  // Admin is now pending for approval
        ...
        pub fun inviteAgent(_ agent: Address) {  // Admin ivites new Agent
            DAAM_V3.agents.insert(key: agent, false ) // Agent account is setup but not active untill accepted.
        ...
        pub fun inviteCreator(_ creator: Address) {  // Admin or Agent invite a new creator
            DAAM_V3.creators.insert(key: creator, false ) // Creator account is setup but not active untill accepted.
        ...
        pub fun inviteMinter(_ minter: Address) {  // Admin invites a new Minter (Key)
            DAAM_V3.minterPending = minter // Minter Key is setup but not active untill accepted.
        ...
        pub fun removeAdminInvite() { // Remove Admin invitation
            DAAM_V3.adminPending = nil  // Clear Admin for pending
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            DAAM_V3.creators[creator] = status // status changed
        ...
        pub fun removeAdmin(admin: Address) { // Two Admin to Remove Admin
            ...
            if self.remove.length >= 2 {
                //self.status = false
                DAAM_V3.admins.remove(key: admin) // Remove selected Admin
                log("Removed Admin")
        ...
        pub fun removeAgent(agent: Address) { // Admin removes selected Agent by Address
            DAAM_V3.agents.remove(key: agent)    // Remove Agent from list
            log("Removed Agent")
            emit AgentRemoved(agent: agent)
        }

        pub fun removeCreator(creator: Address) { // Admin removes selected Creator by Address
            DAAM_V3.creators.remove(key: creator)    // Remove Creator from list
            DAAM_V3.creatorCap.remove(key: creator)  // Remove Creator Capability
            log("Removed Creator")
        ...
        pub fun changeCopyright(mid: UInt64, copyright: CopyrightStatus) {
            DAAM_V3.copyright[mid] = copyright    // Change to new copyright
        ...
        pub fun changeMetadataStatus(mid: UInt64, status: Bool) {
            DAAM_V3.metadata[mid] = status // change to a new Metadata status
        }
	}
/************************************************************************/
...
/************************************************************************/
    pub resource Minter {
        ...
        pub fun mintNFT(metadata: @MetadataHolder): @DAAM_V3.NFT {
            ...
            let request <- DAAM_V3.request.remove(key: mid)! // Get Request using MID
            ...
            if isLast {
                ...
                let empty_request <- DAAM_V3.request.insert(key: mid, <- request) // re-insert request
        ...
        pub fun notNew(tokenID: UInt64) {
            pre  { DAAM_V3.newNFTs.contains(tokenID)  : "This NFT is not a new NFT" }
            post { !DAAM_V3.newNFTs.contains(tokenID) : "Illegal Operation: notNew" } // Unreachable
            var counter = 0 as UInt64              // start the conter
            for nft in DAAM_V3.newNFTs {              // cycle through 'new' list
                if nft == tokenID {                // if Token ID is found
                    DAAM_V3.newNFTs.remove(at: counter) // remove from 'new' list
                    break
                } else {
        ...
        priv fun newNFT(id: UInt64) {
            pre  { !DAAM_V3.newNFTs.contains(id) : "Token ID is already set to New." }
            post { DAAM_V3.newNFTs.contains(id)  : "Illegal Operation: newNFT" }
                DAAM_V3.newNFTs.append(id)       // Append 'new' list
        }        
    }
/************************************************************************/
    ...
    pub fun answerAdminInvite(newAdmin: AuthAccount, submit: Bool): @Admin? {
        pre {
            DAAM_V3.creators[newAdmin.address] == nil : "An admin can not use the same address as an Creator."
            DAAM_V3.agents[newAdmin.address] == nil   : "An admin can not use the same address as an Agent."
        ...
        DAAM_V3.adminPending = nil    // Release Admin pending (nil)
    ...
    {
        pre {
            !DAAM_V3.admins.containsKey(newAgent.address)   : "A Agent can not use the same address as an Admin."
            !DAAM_V3.creators.containsKey(newAgent.address) : "A Agent can not use the same address as an Creator."
            DAAM_V3.agents.containsKey(newAgent.address)    : "You got no DAAM Agent invite."
        ...
        if !submit {                                  // Refused invation. 
            DAAM_V3.agents.remove(key: newAgent.address) // Remove potential from Agent list
            return nil                                // Return and end function
        }
        // Invitation accepted at this point
        DAAM_V3.agents[newAgent.address] = submit        // Add Agent & set Status (True)
    ...
    pub fun answerCreatorInvite(newCreator: AuthAccount, submit: Bool): @Creator? {
        pre {
            ...
            !DAAM_V3.agents.containsKey(newCreator.address)    : "A Creator can not use the same address as an Agent."
        ...
        if !submit {                                       // Refused invation.
            DAAM_V3.creators.remove(key: newCreator.address)  // Remove potential from Agent list
            return nil                                     // Return and end function
        }
        // Invitation accepted at this point
        DAAM_V3.creators[newCreator.address] = submit         // Add Creator & set Status (True) 
    ...
    pub fun answerMinterInvite(minter: AuthAccount, submit: Bool): @Minter? {
        pre { DAAM_V3.minterPending == minter.address : "You do not have a Minter Invitation" }
