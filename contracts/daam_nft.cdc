...
/************************************************************************/
pub contract DAAM_V12: NonFungibleToken {
    // Events
    ...
    pub let agency : MetadataViews.Royalties  // DAAM_V12 Ageny Address
/***********************************************************************/
...
pub resource Request {
    ...
    init(mid: UInt64) {
        ...
        DAAM_V12.metadata[self.mid] != false // Can set a Request as long as the Metadata has not been Disapproved as oppossed to Aprroved or Not Set.
...
pub resource RequestGenerator {
    ...
    pub fun acceptDefault(mid: UInt64, metadataGen: &MetadataGenerator{MetadataGeneratorPublic}, percentage: UFix64) {
        pre {
            ...
            DAAM_V12.creators.containsKey(self.grantee) : "Account: ".concat(self.grantee.toString()).concat("You are not a Creator")
            DAAM_V12.isCreator(self.grantee) == true    : "Account: ".concat(self.grantee.toString()).concat("Your Creator account is Frozen.")
            percentage >= 0.1 && percentage <= 0.3  : "Percentage must be inbetween 10% to 30%."
        }
        // Getting Agency royalties
        //let agency   = DAAM_V12.agency.getRoyalties()
        ...
        let old <- DAAM_V12.request.insert(key: mid, <-request) // advice DAAM_V12 of request
...
/************************************************************************/
    pub resource Metadata {  // Metadata struct for NFT, will be transfered to the NFT.
        ...
        {            
            pre {
                DAAM_V12.validInteract(interact) : "This Interaction is not Authorized"
            ...
            if metadata == nil {
                DAAM_V12.metadataCounterID = DAAM_V12.metadataCounterID + 1
                self.mid         = DAAM_V12.metadataCounterID // init MID with counter
                self.creatorInfo = creators!               // creator of NFT
...
pub resource MetadataGenerator: MetadataGeneratorPublic, MetadataGeneratorMint {
        ...
        init(_ grantee: Address) {
            ...
            DAAM_V12.metadataCap.insert(key: self.grantee, getAccount(self.grantee).getCapability<&MetadataGenerator{MetadataGeneratorPublic}>(DAAM_V12.metadataPublicPath))
        }
            ...
            thumbnail: {String:{MetadataViews.File}}, file: {String:MetadataViews.Media}, interact: AnyStruct? ): UInt64
        {
            ...
            let metadata <- create Metadata(creators: DAAM_V12.creators[self.grantee], name: name, max: max, categories: categories, inCollection: inCollection,
            ...
            DAAM_V12.metadata.insert(key: mid, false)   // a metadata ID for Admin approval, currently unapproved (false)
            DAAM_V12.copyright.insert(key: mid, CopyrightStatus.UNVERIFIED) // default copyright setting

            DAAM_V12.metadata[mid] = true // TODO REMOVE AUTO-APPROVE AFTER DEVELOPMENT
        ...
        pub fun removeMetadata(mid: UInt64) {
            ...
            let old_request <- DAAM_V12.request.remove(key: mid)  // Get Request
        ...
        priv fun clearMetadata(mid: UInt64): @Metadata {            
            DAAM_V12.metadata.remove(key: mid) // Metadata removed from DAAM_V12. Logging no longer neccessary
            DAAM_V12.copyright.remove(key:mid) // remove metadata copyright            
        ...
        pub fun generateMetadata(minter: @MinterAccess, mid: UInt64) : @Metadata {
            pre {
                ...
                self.metadata[mid] != nil : "MetadataID: ".concat(mid.toString()).concat(" does not exist.")
                DAAM_V12.metadata[mid] != nil : "MetadataID: ".concat(mid.toString()).concat(" This already has been published.")
                DAAM_V12.metadata[mid]!       : "MetadataID: ".concat(mid.toString()).concat(" Submission has been Disapproved.")
            }
            destroy minter
...
/************************************************************************/
    pub resource NFT: NonFungibleToken.INFT, INFT, MetadataViews.Resolver {
        ...
        init(metadata: @Metadata, request: &Request?) {
            ...
            DAAM_V12.totalSupply = DAAM_V12.totalSupply + 1 // Increment total supply
            self.id          = DAAM_V12.totalSupply     // Set Token ID with total supply
        ...
        pub fun getCopyright(): CopyrightStatus { // Get current NFT Copyright status
            return DAAM_V12.copyright[self.id]! // return copyright status
        }
...
pub resource interface CollectionPublic {
    pub fun borrowDAAM(id: UInt64): &DAAM_V12.NFT? // Get NFT as DAAM_V12.NFT
...
// Standand Flow Collection Wallet
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic,
        ...
        pub fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                return MetadataViews.NFTCollectionData (
                    storagePath: DAAM_V12.collectionStoragePath,
                    publicPath: DAAM_V12.collectionPublicPath,
                    providerPath: DAAM_V12.collectionPrivatePath,
                    publicCollection: Type<@DAAM_V12.Collection>(),
                    publicLinkedType: Type<&DAAM_V12.Collection{DAAM_V12.CollectionPublic, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, MetadataViews.Resolver}>(),
                    providerLinkedType: ?????, // TODO  ???
                    createEmptyCollectionFunction: (DAAM_V12.createEmptyCollection() : @DAAM_V12.Collection) // TODO ???
                )*/
                default: return nil
        ...
        pub fun borrowViewResolver(id: UInt64): &{MetadataViews.Resolver} {
            ...
            let token <- self.ownedNFTs.remove(key: id)! as! @DAAM_V12.NFT // Get NFT
            let ref = &token as! &DAAM_V12.NFT{MetadataViews.Resolver} // Get reference to NFT
            let oldToken <- self.ownedNFTs[id] <- token   // Store NFT
            destroy oldToken                              // destroy place holder
            return ref //as! &DAAM_V12.NFT{MetadataViews.Resolver}
        }

        // withdraw removes an NFT from the collection and moves it to the caller
        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT {
            self.removeFromPersonalCollection(collectionName: nil, tokenID: withdrawID)
            let token <- self.ownedNFTs.remove(key: withdrawID)! as! @DAAM_V12.NFT // Get NFT
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @DAAM_V12.NFT // Get NFT as DAAM_V12.GFT
        ...
        // borrowDAAM gets a reference to an DAAM_V12.NFT in the album.
        pub fun borrowDAAM(id: UInt64): &DAAM_V12.NFT {
            ...
            return ref as! &DAAM_V12.NFT                                    // return NFT Reference
        }
...
pub resource Admin: Agent
{
        ...
        pub fun newRequestGenerator(): @RequestGenerator {
            pre {
                DAAM_V12.admins[self.owner!.address] == true  : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied. No proper Admin Status")
                self.grantee == self.owner!.address       : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
        ...
        pub fun inviteAdmin(_ admin: Address) {     // Admin invite a new Admin
            pre {
                DAAM_V12.admins[self.owner!.address] == true  : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
                self.grantee == self.owner!.address : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
                self.status                    : "You're no longer a have Access."
                DAAM_V12.creators[admin] == nil : "A Admin can not use the same address as an Creator."
                DAAM_V12.agents[admin] == nil   : "A Admin can not use the same address as an Agent."
                DAAM_V12.admins[admin] == nil   : "They're already sa DAAM_V12 Admin!!!"
                Profile.check(admin) : "You can't be a DAAM_V12 Admin without a Profile! Go make one Fool!!"
            }
            post { DAAM_V12.admins[admin] == false : "Illegal Operaion: inviteAdmin" }

            DAAM_V12.admins.insert(key: admin, false) // Admin account is setup but not active untill accepted.
        ...
        pub fun inviteAgent(_ agent: Address) {    // Admin ivites new Agent
            pre {
                ...
                self.grantee == self.owner!.address : "Account: ".concat(self.owner!.address.toString()).concat(" Permission Denied")
                self.status                 : "You're no longer a have Access."
                DAAM_V12.admins[agent] == nil   : "A Agent can not use the same address as an Admin."
                DAAM_V12.creators[agent] == nil : "A Agent can not use the same address as an Creator."
                DAAM_V12.agents[agent] == nil   : "They're already a DAAM_V12 Agent!!!"
                Profile.check(agent) : "You can't be a DAAM_V12 Admin without a Profile! Go make one Fool!!"
            }

            post {
                DAAM_V12.agents[agent] == false : "Illegal Operaion: invite Agent"
                DAAM_V12.admins[agent] == false : "Illegal Operaion: invite Agent"
            }

            DAAM_V12.admins.insert(key: agent, false) // Admin account is setup but not active untill accepted.
            DAAM_V12.agents.insert(key: agent, false )     // Agent account is setup but not active untill accepted.
        ...
        pub fun inviteCreator(_ creator: Address) {    // Admin or Agent invite a new creator
            pre {
                DAAM_V12.admins[self.owner!.address] == true  : "Permission Denied"
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                   : "You're no longer a have Access."
                DAAM_V12.admins[creator]   == nil : "A Creator can not use the same address as an Admin."
                DAAM_V12.agents[creator]   == nil : "A Creator can not use the same address as an Agent."
                DAAM_V12.creators[creator] == nil : "They're already a DAAM_V12 Creator!!!"
                Profile.check(creator)        : "You can't be a DAAM_V12 Creator without a Profile! Go make one Fool!!"
            }
            post { DAAM_V12.isCreator(creator) == false : "Illegal Operaion: inviteCreator" }
            ...
            let agentArg  : {Address: MetadataViews.Royalty}? = 
                DAAM_V12.isAgent(self.owner!.address) != nil ? {self.owner!.address: agencyRoyalty} : {}
            let creatorInfo = CreatorInfo(
                creator: creatorArg,
                agent: agentArg ,
                status: false) // Store Creator status, false = invited, not yet answered
            DAAM_V12.creators.insert(key: creator, creatorInfo ) // Creator account is setup but not active untill accepted.
        ...
        pub fun inviteMinter(_ minter: Address) {   // Admin invites a new Minter (Key)
            ...
            post { DAAM_V12.minters[minter] == false : "Illegal Operaion: inviteCreator" }

            DAAM_V12.minters.insert(key: minter, false) // Minter Key is setup but not active untill accepted.
        ...
        pub fun removeAdmin(admin: Address) { // Two Admin to Remove Admin
            ...
            DAAM_V12.remove.insert(key: self.grantee, admin) // Append removal list
            if DAAM_V12.remove.length >= vote {                      // If votes is 3 or greater
                var counter: {Address: Int} = {} // {To Remove : Total Votes}
                // Talley Votes
                for a in DAAM_V12.remove.keys {
                    let remove = DAAM_V12.remove[a]! // get To Remove
                    // increment counter
                ...
                for c in counter.keys {
                    if counter[c]! >= vote {        // Does To Remove have enough votes to be removed
                        DAAM_V12.remove = {}           // Reset DAAM_V12.Remove
                        DAAM_V12.admins.remove(key: c) // Remove selected Admin
                        log("Removed Admin")
        ...
        pub fun removeAgent(agent: Address) { // Admin removes selected Agent by Address
            pre  {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                    : "You're no longer a have Access."
                DAAM_V12.agents.containsKey(agent) : "This is not a Agent Address."
            }
            post {
                !DAAM_V12.admins.containsKey(agent) : "Illegal operation: removeAgent"
                !DAAM_V12.agents.containsKey(agent) : "Illegal operation: removeAgent"
            } // Unreachable

            DAAM_V12.admins.remove(key: agent)    // Remove Agent from list
            DAAM_V12.agents.remove(key: agent)    // Remove Agent from list
            log("Removed Agent")
        ...
        pub fun removeCreator(creator: Address) { // Admin removes selected Creator by Address
            pre {  
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                        : "You're no longer a have Access."
                DAAM_V12.creators.containsKey(creator) : "This is not a Creator address."
            }
            post { !DAAM_V12.creators.containsKey(creator) : "Illegal operation: removeCreator" } // Unreachable

            DAAM_V12.creators.remove(key: creator)    // Remove Creator from list
            DAAM_V12.metadataCap.remove(key: creator) // Remove Metadata Capability from list
            log("Removed Creator")
        ...
        pub fun removeMinter(minter: Address) { // Admin removes selected Agent by Address
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                      : "You're no longer a have Access."
                DAAM_V12.minters.containsKey(minter) : "This is not a Minter Address."
            }
            post { !DAAM_V12.minters.containsKey(minter) : "Illegal operation: removeAgent" } // Unreachable
            DAAM_V12.minters.remove(key: minter)    // Remove Agent from list
            log("Removed Minter")
        ...
        pub fun changeAgentStatus(agent: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                     : "You're no longer a have Access."
                DAAM_V12.agents.containsKey(agent)  : "Wrong Address. This is not an Agent."
                DAAM_V12.agents[agent] != status    : "Agent already has this Status."
            }
            post { DAAM_V12.agents[agent] == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            DAAM_V12.agents[agent] = status // status changed
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                         : "You're no longer a have Access."
                DAAM_V12.creators.containsKey(creator)  : "Wrong Address. This is not a Creator."
                DAAM_V12.isCreator(creator) != status    : "Agent already has this Status."
            }
            post { DAAM_V12.isCreator(creator) == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            let creatorInfo = &DAAM_V12.creators[creator]! as &CreatorInfo
        ...
        pub fun changeMinterStatus(minter: Address, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                       : "You're no longer a have Access."
                DAAM_V12.minters.containsKey(minter)  : "Wrong Address. This is not a Minter."
                DAAM_V12.minters[minter] != status    : "Minter already has this Status."
            }
            post { DAAM_V12.minters[minter] == status : "Illegal Operation: changeCreatorStatus" } // Unreachable

            DAAM_V12.minters[minter] = status // status changed
        ...
        pub fun changeMetadataStatus(mid: UInt64, status: Bool) {
            pre {
                ...
                self.grantee == self.owner!.address : "Permission Denied"
                self.status                          : "You're no longer a have Access."
                DAAM_V12.copyright.containsKey(mid)      : "This is an Invalid MID"
            }            
            DAAM_V12.metadata[mid] = status // change to a new Metadata status
        }      
        ...
        pub fun changeCopyright(mid: UInt64, copyright: CopyrightStatus) {
            ...
            post { DAAM_V12.copyright[mid] == copyright  : "Illegal Operation: changeCopyright" } // Unreachable

            DAAM_V12.copyright[mid] = copyright    // Change to new copyright
...
// to Create Metadata which inturn can be made in NFTs after Minting
    pub resource Creator {
        ...
        pub fun newMetadataGenerator(): @MetadataGenerator {
            pre{
                self.grantee == self.owner!.address : "Permission Denied"
                DAAM_V12.creators.containsKey(self.grantee) : "You're not a Creator."
                DAAM_V12.isCreator(self.grantee) == true    : "Your Creator account is Frozen."
            }
...
// Note: new is defined by newly Minted. Age is not a consideration.
    pub resource Minter
    {
        ...
        init(_ minter: Address) {
            self.grantee = minter
            DAAM_V12.minters.insert(key: minter, true) // Insert new Minter in minter list.
        }

        pub fun mintNFT(metadata: @Metadata): @DAAM_V12.NFT {
            pre{
                //metadata.edition.number <= metadata.edition.max || metadata.edition == 0 : "Internal Error: Mint Counter"
                DAAM_V12.isCreator(metadata.creatorInfo.creator.keys[0]) == true :
                    "Account: ".concat(metadata.creatorInfo.creator.keys[0].toString()).concat(" This is not our Creator.")
                DAAM_V12.isMinter(self.grantee) == true    : "Account: ".concat(self.grantee.toString()).concat(" Your Creator account is Frozen.")
                DAAM_V12.request.containsKey(metadata.mid) : "Invalid Request"
            }
            var isLast = false
            ...
            let nft <- create NFT(metadata: <- metadata, request: &DAAM_V12.request[mid] as &Request) // Create NFT

            // Update Request, if last remove.
            if isLast {
                let request <- DAAM_V12.request.remove(key: mid)! // Get Request using MID
        ...
        pub fun notNew(tokenID: UInt64) {
            pre  {
                self.grantee == self.owner!.address : "Permission Denied"
                DAAM_V12.newNFTs.contains(tokenID)      : "Token ID: ".concat(tokenID.toString()).concat(" is Not New.")
            }
            post { !DAAM_V12.newNFTs.contains(tokenID) : "Illegal Operation: notNew" } // Unreachable

            var counter: UInt64 = 0 as UInt64              // start the conter
            for nft in DAAM_V12.newNFTs {              // cycle through 'new' list
                if nft == tokenID {                // if Token ID is found
                    DAAM_V12.newNFTs.remove(at: counter) // remove from 'new' list
                    break
                } else {
        ...
        priv fun newNFT(id: UInt64) {
            pre  { !DAAM_V12.newNFTs.contains(id) : "Token ID: ".concat(id.toString()).concat(" is already set to New.") }
            post { DAAM_V12.newNFTs.contains(id)  : "Illegal Operation: newNFT" }
                DAAM_V12.newNFTs.append(id)       // Append 'new' list
        }        
    }
/************************************************************************/
pub resource MinterAccess 
{
    ...
    pub fun validate(): Bool { return DAAM_V12.minters[self.minter]==true ? true : false }
}
/************************************************************************/
    // Public DAAM_V12 functions
    ...
    pub fun answerAdminInvite(newAdmin: AuthAccount, submit: Bool): @Admin? {
        pre {
            ...
            self.isAdmin(newAdmin.address) == false : "Account: ".concat(newAdmin.address.toString()).concat(" You got no DAAM_V12 Admin invite.")
            Profile.check(newAdmin.address)  : "You can't be a DAAM_V12 Admin without a Profile first. Go make a Profile first."
        }

        if !submit { 
            DAAM_V12.admins.remove(key: newAdmin.address) // Release Admin
            return nil
        }  // Refused invitation. Return and end function
        
        // Invitation accepted at this point
        DAAM_V12.admins[newAdmin.address] = submit // Insert new Admin in admins list.
        log("Admin: ".concat(newAdmin.address.toString()).concat(" added to DAAM_V12") )
    ...
    pub fun answerAgentInvite(newAgent: AuthAccount, submit: Bool): @Admin{Agent}?
    {
        pre {
            ...
            self.isAgent(newAgent.address)   == false : "Account: ".concat(newAgent.address.toString()).concat(" You got no DAAM_V12 Agent invite.")
            Profile.check(newAgent.address) : "You can't be a DAAM_V12 Agent without a Profile first. Go make a Profile first."
        }

        if !submit {                                  // Refused invitation. 
            DAAM_V12.admins.remove(key: newAgent.address) // Remove potential from Agent list
            DAAM_V12.agents.remove(key: newAgent.address) // Remove potential from Agent list
            return nil                                // Return and end function
        }
        // Invitation accepted at this point
        DAAM_V12.admins[newAgent.address] = submit        // Add Admin & set Status (True)
        DAAM_V12.agents[newAgent.address] = submit        // Add Agent & set Status (True)

        log("Agent: ".concat(newAgent.address.toString()).concat(" added to DAAM_V12") )
    ...
    pub fun answerCreatorInvite(newCreator: AuthAccount, submit: Bool): @Creator? {
        pre {
            ...
            self.isCreator(newCreator.address) == false : "Account: ".concat(newCreator.address.toString()).concat(" You got no DAAM_V12 Creator invite.")
            Profile.check(newCreator.address) : "You can't be a DAAM_V12 Creator without a Profile first. Go make a Profile first."
        }

        if !submit {                                       // Refused invitation.
            DAAM_V12.creators.remove(key: newCreator.address)  // Remove potential from Agent list
            return nil                                     // Return and end function
        }
        // Invitation accepted at this point
        let creatorInfo = &DAAM_V12.creators[newCreator.address]! as &CreatorInfo
        creatorInfo.status = submit         // Add Creator & set Status (True)
        log("Creator: ".concat(newCreator.address.toString()).concat(" added to DAAM_V12") )
    ...
    pub fun answerMinterInvite(newMinter: AuthAccount, submit: Bool): @Minter? {
        ...
        if !submit {                                      // Refused invitation. 
            DAAM_V12.minters.remove(key: newMinter.address) // Remove potential from Agent list
            return nil                                    // Return and end function
        }
        // Invitation accepted at this point
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        ...
        return <- create DAAM_V12.Collection() // Return Collection Resource
    }

    // Create an new Collection to store NFTs
    /*pub fun createDAAM_V12Collection(): @DAAM_V12.Collection {
        post { result.getIDs().length == 0: "The created DAAM_V12 collection must be empty!" }
        return <- create DAAM_V12.Collection() // Return Collection Resource
    }*/
...
/************************************************************************/
// Init DAAM_V12 Contract variables
    ...
    init(founders: {Address:UFix64}, defaultAdmins: [Address])
    {
        ...
        // Paths
        self.collectionPublicPath  = /public/DAAM_V12_Collection
        self.collectionStoragePath = /storage/DAAM_V12_Collection
        self.metadataPublicPath    = /public/DAAM_V12_SubmitNFT
        self.metadataStoragePath   = /storage/DAAM_V12_SubmitNFT
        self.adminPrivatePath      = /private/DAAM_V12_Admin
        self.adminStoragePath      = /storage/DAAM_V12_Admin
        self.minterPrivatePath     = /private/DAAM_V12_Minter
        self.minterStoragePath     = /storage/DAAM_V12_Minter
        self.creatorPrivatePath    = /private/DAAM_V12_Creator
        self.creatorStoragePath    = /storage/DAAM_V12_Creator
        self.requestPrivatePath    = /private/DAAM_V12_Request
        self.requestStoragePath    = /storage/DAAM_V12_Request
