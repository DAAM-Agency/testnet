...
/************************************************************************/
pub contract DAAM_V2: NonFungibleToken {
...
pub resource Request {
    ...
    access(contract) fun bargin(signer: AuthAccount, mid: UInt64, royality: {Address:UFix64} ) {
        // Verify is Creator
        pre {
            signer.borrow<&{DAAM_V2.Founder}>(from: DAAM_V2.adminStoragePath) != nil ||
            signer.borrow<&DAAM_V2.Creator>(from: DAAM_V2.creatorStoragePath) != nil : "You do not have access."

            royality.containsKey(DAAM_V2.agency) : "Agency must be included in Royality."
            !self.isValid() : "Neogoation is already closed. Both parties have already agreed."
        }
        // 0 = creator, 1 = admin
        let selected   = signer.borrow<&DAAM_V2.Creator>(from: DAAM_V2.creatorStoragePath) != nil ? 0 : 1
        let unselected = signer.borrow<&DAAM_V2.Creator>(from: DAAM_V2.creatorStoragePath) != nil ? 1 : 0
...
pub resource RequestGenerator {
    ...
    pub fun createRequest(signer: AuthAccount, metadata: &Metadata, royality: {Address : UFix64} ) {
        pre {
            !DAAM_V2.request.containsKey(metadata.mid) : "Already made request for this MID."
            metadata != nil
            royality.length > 1

            signer.borrow<&DAAM_V2.Creator>(from: DAAM_V2.creatorStoragePath) != nil ||
            signer.borrow<&{DAAM_V2.Founder}>(from: DAAM_V2.adminStoragePath) != nil : "You do not have access"
        }
        let mid = metadata.mid
        let request <-! create Request(metadata: metadata)!
        request.bargin(signer: signer, mid: mid, royality: royality)

        let old <- DAAM_V2.request.insert(key: mid, <-request) // advice DAAM of request
        destroy old
    ...
    pub fun acceptDefault(creator: AuthAccount, metadata: &Metadata) {
        let mid = metadata.mid
        var royality = {DAAM_V2.agency: 0.05 as UFix64 }
    ...
    pub fun getRequest(metadata: &MetadataHolder): @Request {
        pre {
            metadata != nil
            DAAM_V2.request.containsKey(metadata.getMID() )
            DAAM_V2.getRequestValidity(mid: metadata.getMID() )
        }
        let request <- DAAM_V2.request.remove(key: metadata.metadata.mid)!
        return <- request
    }
}
/************************************************************************/
    pub struct Metadata {  // Metadata for NFT,metadata initialization
        ...
        init(creator: Address, series: UInt64, data: String, thumbnail: String, file: String, counter: UInt64) {
            ...
            self.mid       = DAAM_V2.metadataCounterID
...
pub resource MetadataGenerator {
        ...
        pub fun addMetadata(series: UInt64, data: String, thumbnail: String, file: String) {
            ...
            DAAM_V2.metadata.insert(key: metadata.mid, false)
            DAAM_V2.copyright.insert(key:metadata.mid, CopyrightStatus.UNVERIFIED)

            log("Metadata Generatated ID: ".concat(metadata.mid.toString()) )
            emit DAAM_V2.MetadataGeneratated()
        }
...
/************************************************************************/
    pub resource NFT: NonFungibleToken.INFT {
        ...
        init(metadata: @MetadataHolder, request: @Request) {
            ...
            DAAM_V2.totalSupply = DAAM_V2.totalSupply + 1 as UInt64
            self.id = DAAM_V2.totalSupply
        ...
        pub fun getCopyright(): CopyrightStatus {
            return DAAM_V2.copyright[self.id]!
        }
    }
/************************************************************************/
pub resource interface CollectionPublic {
    ...
    pub fun borrowDAAM(id: UInt64): &DAAM_V2.NFT
}     
/************************************************************************/
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, CollectionPublic {
        ...
        init() {
            self.ownedNFTs <- {}
            DAAM_V2.collectionCounterID = DAAM_V2.collectionCounterID + 1 as UInt64
            self.id = DAAM_V2.collectionCounterID
        }
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @DAAM_V2.NFT
...
/************************************************************************/
    pub resource interface Founder {
        pub fun inviteAdmin(newAdmin: Address) {
            pre{
                DAAM_V2.adminPending == nil : "Admin already pending. Waiting on confirmation."
                Profile.check(newAdmin)  : "You can't add DAAM Admin without a Profile! Tell'em to make one first!!"
            }
            post { DAAM_V2.adminPending != nil : "We're being hacked or something" }
        }

        pub fun inviteCreator(_ creator: Address) {  // Admin add a new creator
            pre {
                DAAM_V2.creators.containsKey(creator) == false : "They're already a DAAM Creator!!!"
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            pre {
                DAAM_V2.creators.containsKey(creator) : "They're no DAAM Creator!!!"
                DAAM_V2.creators[creator] != status   : "Creator already has this status."
            }
            post { DAAM_V2.creators[creator] == status}
        }

        pub fun removeCreator(creator: Address) {
            pre  { DAAM_V2.creators.containsKey(creator) : "They're no DAAM Creator!!!" }
            post { !DAAM_V2.creators.containsKey(creator) }
        }

        pub fun removeAdmin(admin: Address)

        pub fun changeCopyright(mid: UInt64, copyright: CopyrightStatus) {
            pre  { DAAM_V2.copyright.containsKey(mid): "This is an Invalid MID" }
            post { DAAM_V2.copyright[mid] == copyright }
        }
...
/************************************************************************/
	pub resource Admin: Founder
    {
        ...
        init(_ admin: AuthAccount) {
            self.status = true
            self.remove = []
            DAAM_V2.admins.insert(key: admin.address, true)
        }
        ...
        pub fun inviteAdmin(newAdmin: Address) {
            pre{
                ...
                DAAM_V2.creators[newAdmin] == nil: "An Admin can not use the same address as a Creator."
            }
            DAAM_V2.adminPending = newAdmin
        ...
        pub fun inviteCreator(_ creator: Address) {  // Admin add a new creator
            pre{ self.status : "You're no longer a DAAM Admin!!" }
            DAAM_V2.creators.insert(key: creator, false )
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool) {
            ...
            DAAM_V2.creators[creator] = status
        ...
        pub fun removeCreator(creator: Address) {
            pre{ self.status : "You're no longer a DAAM Admin!!" }            
            DAAM_V2.creators.remove(key: creator)
            log("Removed Creator")
        ...
        pub fun removeAdmin(admin: Address) {
            ...
            if self.remove.length >= 2 {
                ...
                DAAM_V2.admins.remove(key: admin)
                log("Removed Admin")
        ...
        pub fun changeCopyright(mid: UInt64, copyright: CopyrightStatus) {
            pre{ self.status : "You're no longer a DAAM Admin!!" }
            DAAM_V2.copyright[mid] = copyright
...
/************************************************************************/
    pub resource Creator {
        ...
        pub fun newMetadataGenerator(): @MetadataGenerator {
            pre{
                DAAM_V2.creators.containsKey(self.owner?.address!) : "You're not a Creator."
                DAAM_V2.creators[self.owner?.address!] == true     : "This Creators' account is Frozen."
            }
...
/************************************************************************/
    pub resource Minter {
        ...
        pub fun mintNFT(metadata: @MetadataHolder): @DAAM_V2.NFT {
            pre{
                DAAM_V2.creators.containsKey(metadata.metadata.creator) : "You're not a Creator."
                DAAM_V2.creators[metadata.metadata.creator] == true     : "This Creators' account is Frozen."
                DAAM_V2.request.containsKey(metadata.metadata.mid)      : "Invalid Request"
                DAAM_V2.getRequestValidity(mid: metadata.metadata.mid)  : "There is no Request for this MID."
            }
            let request <- DAAM_V2.request.remove(key: metadata.metadata.mid)!
        ...
        pub fun notNew(tokenID: UInt64) {
            pre  { DAAM_V2.newNFTs.contains(tokenID)  }
            post { !DAAM_V2.newNFTs.contains(tokenID) }
            var counter = 0
            for nft in DAAM_V2.newNFTs {
                if nft == tokenID {
                    DAAM_V2.newNFTs.remove(at: counter)
                    break
                } else {
                    counter = counter + 1
                }
        ...
        priv fun newNFT(id: UInt64) {
            pre  { !DAAM_V2.newNFTs.contains(id) }
            post { DAAM_V2.newNFTs.contains(id)  }
                DAAM_V2.newNFTs.append(id)
        }        
    }
/************************************************************************/
    // DAAM Functions
    ...
    pub fun answerAdminInvite(newAdmin: AuthAccount, submit: Bool): @Admin{Founder} {
        pre {
            DAAM_V2.creators[newAdmin.address] == nil: "An Admin can not use the same address as a Creator."
            DAAM_V2.adminPending == newAdmin.address : "You got no DAAM Admin invite."
    ...
    pub fun answerCreatorInvite(newCreator: Address, submit: Bool): @Creator? {
        ...
        if !submit {
            DAAM_V2.creators.remove(key: newCreator)
            return nil
        }      
        log(self.account.address.toString())
        DAAM_V2.creators[newCreator] = submit        
    ...
    pub fun answerMinterInvite(minter: Address, submit: Bool): @Minter {
        pre { DAAM_V2.minterPending == minter }
        DAAM_V2.minterPending = nil
        if !submit { panic("Thank you for your consideration.") }        
