...
grep -rl fd43f9148d4b725d --exclude-dir='.*' --exclude={*.sh,*.md} | xargs sed -i 's|fd43f9148d4b725d|a4ad5ea5c0bd2fba|g' # DAAM_V21 Contract
grep -rl 0ae53cb6e3f42a79 --exclude-dir='.*' --exclude={*.sh,*.md} | xargs sed -i 's|0ae53cb6e3f42a79|7e60df042a9c0868|g' # Flow Token

grep -rl " DAAM" --exclude-dir='.*' --exclude={*.sh,*.md} | xargs sed -i 's| DAAM| DAAM_V22|g' # 
grep -rl 'DAAM\.' --exclude-dir='.*' --exclude={*.sh,*.md} | xargs sed -i 's|DAAM.|DAAM_V22.|g' # 

echo "Manually Verify last line of Auction Contract, verify FUSD Address"
grep -rl AuctionHouse --exclude-dir='.*' --exclude={*.sh,*.md} | xargs sed -i 's|AuctionHouse|AuctionHouse_V15|g' # 
