...
pub struct Data {
        ...
        pub let metadata           : DAAM_V8.MetadataHolder?

        init(metadata: DAAM_V8.MetadataHolder?) {
            self.auctionID = 0
            self.mid = 0
            self.start = 0.0
            self.length = 0.0
...
pub fun getData(auction: &AuctionHouse_V2.Auction, metadata: DAAM_V8.MetadataHolder?): Data {
...
pub fun main(): {Address : [Data] } {    
    let auctionList = AuctionHouse_V2.getCurrentAuctions() // get Auctioneers and AIDs {Address : [aid]}
    ...
    for auctionieer in auctionList.keys {
        ...
        for aid in auctionList[auctionieer]! {
            let AuctionHouse_V2 = getAccount(auctionieer)
            .getCapability<&{AuctionHouse_V2.AuctionPublic}>(AuctionHouse_V2.auctionPublicPath)
            .borrow()!

            let metadata = AuctionHouse_V2.item(aid).itemInfo() 

            convertAuctionList.append(
                getData(auction: AuctionHouse_V2.item(aid), metadata: metadata)
            ) // Save converted data
        }
